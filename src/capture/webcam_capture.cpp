...
WebcamCapture::WebcamCapture(const std::string& device_path) : device_path_(device_path), fd_(-1) {}
...
bool WebcamCapture::open_device() {
    ...
    buffers_.resize(1);
    buffers_[0].start = buffer;
    buffers_[0].length = buf.length;
...
std::optional<utils::Image> WebcamCapture::capture_frame() {
    ...
    const unsigned char* yuyv = (const unsigned char*)buffers_[0].start;
